...
Both charts (altitude + all-sky/azimuthal), event times, the linked time slider, observatory CRUD with JSON export/import, a Help dialog, the Moon (both as an optional overlay and as a selectable target — trajectory/rise/set/phase), and an embedded Aladin Lite sky viewer for deep-sky targets are all in. The app is built and deployed to **skypath.voronin.cc** (subdomain, decided in phase 9). See `.plan/state.md` for the authoritative per-phase table.
...
- `src/lib/images/` — `aladinViewParams` computes the Aladin Lite target/fov/survey for the sky view shown above the object title (deep-sky only). Pure computation only; the app never loads Aladin itself from here (see `aladinLoader.ts`, below). RA is catalogued in **hours** and the target string wants **degrees** (×15), and coordinates must be the **J2000 catalogue** ones, not `equatorialPosition`'s of-date output. Default survey is `P/DSS2/color`.
- `src/components/` — Svelte UI, laid out per `.plan/ui-mocks.md`: observatory list left, Search / Results tabview right.
  ...
  - `ObjectSkyView` is the collapsible Aladin Lite block. It takes ready-made `target`/`fov`/`survey` (the panel builds them via `aladinViewParams`) and only mounts a container **while open**, so a collapsed block costs no CDN/WASM download. Loading Aladin itself goes through the injectable `loadAladin` prop (default `loadAladinView` from `./aladinLoader.ts`, which injects the CDN `<script>`, checks WebGL2 first, and times out — Aladin Lite has no documented tile-load-failure event, unlike the old SkyView `<img>`'s `onerror`) — tests inject a fake loader instead, the same pattern `ObservatoryManager` uses for its `store` prop. Aladin has no API to re-target or destroy an existing instance, so a changed object or a Retry remounts a fresh container via a keyed block rather than reusing one. The visual project must never let this reach aladin.cds.unistra.fr — `setup-visual.ts` collapses it and the dedicated test injects a fake `loadAladin` that paints synthetic DOM.
