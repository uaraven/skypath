...
A static single-page web app for planning a night of astronomical observation. Pick a target, a date and an observatory, and see where the object goes across the sky that night — against *your* horizon, not an idealised flat one.
...
Non-events are answers too: *circumpolar — always up*, *never rises*, *stays blocked*, *polar day*. The app says which rather than printing a dash.

**Targets** — the full Messier catalogue (110 objects) and the solar system planets, searchable by any designation or common name (`M13`, `messier 13`, `NGC 6205`, bare `6205`, `Hercules`). The catalogue model is multi-catalog by design: an object belongs to many catalogues and carries many names, so NGC or Sharpless can be added without duplicating what Messier already covers.
...
npm run catalog:build   # regenerate catalogue JSON from OpenNGC
```
...
src/lib/catalog/     Messier + solar system, search, multi-catalog merge
...
Deep-sky data is generated from [OpenNGC](https://github.com/mattiaverga/OpenNGC), licensed **CC-BY-SA-4.0** — attribution is a licence obligation and is shown in the app's credits. The JSON under `src/lib/catalog/data/` is generated by `npm run catalog:build`; don't hand-edit it.

## Known limitations

- Times display in the browser's local timezone.
- The Moon as a *target* is not finished (its trajectory is drawn from the body centre, while the moonrise row correctly uses the upper limb — the two read a couple of minutes apart).
